...
def chat():
    return render_template("gemini_api_2.html", chat_history=chat_history)

@gemini_api_bp.route("/chat/<name>")
def chat2(name):
    return render_template("gemini_api_2.html", name=name, chat_history=chat_history)

@gemini_api_bp.route("/stream", methods=["POST"])
def start_chat():
    prompt = request.form["prompt"]
    # check prompt
    kamblok_prompt = f"'{prompt}' Is this topic related to a lesson plan? If yes, please provide a brief overview or description of the lesson plan. If not, respond with a general answer about the topic."
